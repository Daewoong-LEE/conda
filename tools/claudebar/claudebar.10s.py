...
# ── xbar / SwiftBar output ────────────────────────────────────────────────────
# ---- Status bar (single line) -----------------------------------------------
sep = " · " if mins_remaining > 0 else ""
mins_label = f"{mins_remaining}m" if mins_remaining > 0 else ""
...
# Three columns (simulate with monospaced)
print(f"{'Input':<12}{'Output':<12}Cache | size=12 color=white bold=true font=Menlo")
inp_s   = _fmt(inp)
out_s   = _fmt(out)
cache_s = _fmt(cw + cr)
print(f"{inp_s:<12}{out_s:<12}{cache_s} | size=14 font=Menlo color=white")
print("---")
...
if by_model:
    ...
    for mdl, v in sorted(by_model.items(), key=lambda x:-(x[1][0]+x[1][1])):
        short = (mdl.replace("claude-","")
                    ...
                    .replace("-20240307",""))
        tot = v[0]+v[1]
        print(f"  {short:<26} {_fmt(tot):>7}   ${v[4]:.3f} | size=12 font=Menlo color=white")
    print("---")
